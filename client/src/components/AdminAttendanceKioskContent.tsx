...
type AdminAttendanceKioskContentProps = {
  mode?: "embedded" | "fullscreen";
};

function getEventLabel(eventType: "check_in" | "check_out") {
  return eventType === "check_in" ? "등원" : "하원";
}

function getEventVariant(eventType: "check_in" | "check_out") {
  return eventType === "check_in" ? "success" : "info";
}

export default function AdminAttendanceKioskContent({
  mode = "embedded",
}: AdminAttendanceKioskContentProps) {
  ...
  useEffect(() => {
    if (inputBuffer.length !== 4 || recordMutation.isPending) {
      return;
    }
  ...
  const feedCardHeight = isFullscreenMode
    ? "min-h-[560px] xl:min-h-[780px]"
    : "min-h-[560px] xl:min-h-[720px]";
  const keypadButtonClass = isFullscreenMode
    ? "h-24 rounded-3xl text-2xl font-semibold transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 xl:h-28 xl:text-3xl"
    : "h-20 rounded-3xl text-xl font-semibold transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 sm:h-24 sm:text-2xl";
  const bufferClass = isFullscreenMode
    ? "mt-4 text-5xl font-bold tracking-[0.4em] md:text-6xl md:tracking-[0.55em]"
    : "mt-4 text-4xl font-bold tracking-[0.35em] md:text-5xl md:tracking-[0.6em]";

  return (
    <div className="space-y-6">
      <div
        className={`grid grid-cols-2 gap-4 ${isFullscreenMode ? "2xl:grid-cols-4" : "lg:grid-cols-4"}`}
      >
      ...
      <div
        className={`grid grid-cols-1 gap-6 ${isFullscreenMode ? "2xl:grid-cols-[minmax(0,1.15fr)_minmax(440px,0.85fr)]" : "xl:grid-cols-2"}`}
      >
        <Card variant="elevated" padding="lg" className={feedCardHeight}>
          ...
          <div className="mt-6 space-y-3">
            {isLoading ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant={getEventVariant(item.eventType)} size="sm">
              ...
              <EmptyState
                title="오늘 기록이 아직 없습니다."
                description="오른쪽 키패드에 학생 출석번호 4자리를 입력해 주세요."
              />
            )}
          </div>
        </Card>
        ...
        <Card variant="elevated" padding="lg" className={feedCardHeight}>
          <div className="space-y-2">
            ...
            <p className="text-sm" style={{ color: theme.colors.text.tertiary }}>
              출석번호 4자리가 채워지면 자동으로 등하원이 기록됩니다.
            </p>
          </div>
          ...
          >
            ...
            <ul className="mt-3 space-y-2 text-sm" style={{ color: theme.colors.text.tertiary }}>
              ...
              <li>같은 날 등원과 하원이 모두 있으면 더 이상 입력할 수 없습니다.</li>
              <li>등록되지 않은 번호는 오류로 안내됩니다.</li>
              <li>PC 키보드 숫자 입력과 백스페이스, 삭제 키도 사용할 수 있습니다.</li>
            </ul>
          </div>
        </Card>
      </div>
